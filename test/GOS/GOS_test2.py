...
def center_calc(points_df):     #points_df에 x,y 좌표 가져옴
    x = points_df.x
    y = points_df.y

    X = (max(x)+min(x))/2.      #중심좌표 구하기 실수 나눗셈
    Y = (max(y)+min(y))/2.
...
state_path = 'test/GOS/CTPRVN_wgs84.json'
state_geo = json.load(open(state_path, encoding='utf-8'))

state_unemployment = 'test/GOS/Total_People_2020_6_2.csv'
...
target_df = pd.merge(state_data, center_locations, how='left', on='CODE') #기준점이 on 값이라는 것을 알려줌, 데이터 합치기
target_df = target_df[~np.isnan(target_df['X'])]  # 위치 정보 없는 값 제외 isnan 배열안에 NUM 값이 있니 물어보는 함수
